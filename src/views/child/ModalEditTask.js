...
export default function ModalEditTask({
    ...
    const [newTask, setNewTask] = useState({ name: task.name, progress: task.progress_percentage });

    function editTask() {
        if (!task.todo_id || !task.id || !newTask.name || !newTask.progress) {
            alert("Task ID, Group ID, name, and progress must not empty!");
            return;
        }
        API.patchUpdateItem(task.todo_id, task.id, newTask.name)
            .then(() => {
                alert(newTask.name + " - Task Edited!");
